...
async def forward(ctx):
    source_location = bot.get_channel(ctx.channel.id)
    sending_location = bot.get_channel(949622979850018826)
    await ctx.send(sending_location)
    await ctx.send(source_location)
    messages = []
    await bot.wait_until_ready()
    async for i in source_location.history(limit=200).flatten():
        if i.content:
            messages.append(i.content)
    async for i in messages:
        await sending_location.send(str(i))
